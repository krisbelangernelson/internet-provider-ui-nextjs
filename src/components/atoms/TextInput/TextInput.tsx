...
export default function TextInput(props: TextInputProps) {
  const { label, name, type, placeholder, onChange, value, isValid, isInvalid, feedback, dataTestid } = props
  ...
  return (
    <div>
      ...
      <input type={type} id={name} name={name} placeholder={placeholder} className={`border text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 ${color}`} onChange={onChange} value={value} data-testid={dataTestid ?? 'input'} />
